...
@section('content')
    <div class="container mx-auto py-8">
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6">
                ...
                <p class="mt-1 max-w-2xl text-sm text-gray-500">{{ __('User information') }}</p>
                <div class="flex items-center space-x-4 py-5">
                    @auth
                        @if(auth()->user()->id == $user->id)
                            <img  data-modal-target="staticModal" data-modal-toggle="staticModal" class="w-10 h-10 rounded-full hover:border-blue-800" src="{{ Storage::url($user->avatar) }}" alt="">
                        @else
                            <img class="w-10 h-10 rounded-full" src="{{ Storage::url($user->avatar) }}" alt="">
                        @endif
                    @endauth
                    <div class="font-medium dark:text-white">
                        <div>{{ $user->username }}</div>
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            {{ __('Joined on') }} {{ $user->created_at->format('F Y') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <article class="bg-white shadow overflow-hidden sm:rounded-lg p-5">
        <div class="flex items-center mb-4 space-x-4">
            <img class="w-10 h-10 rounded-full" src="{{ Storage::url($user->avatar) }}" alt="">
            <div class="space-y-1 font-medium dark:text-white">
                <p>{{ $user->username }} <time datetime="2014-08-16 19:00" class="block text-sm text-gray-500 dark:text-gray-400">Joined on August 2014</time></p>
            </div>
        </div>
